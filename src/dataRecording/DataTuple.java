...
public class DataTuple {
	...
	public float normalizeDistance(int dist)
	{
		...
		return ((dist2-0)/((float)this.numberOfNodesInLevel-0))*(1-0)+0;
	}
	
	public float normalizeLevel(int level)
	{
		return (((float)level-0)/((float)Constants.NUM_MAZES-0))*(1-0)+0;
	}
	
	public float normalizePosition(int position)
	{
		
		return (((float)position-0)/((float)this.numberOfNodesInLevel-0))*(1-0)+0;
	}
	
	public float normalizeBoolean(boolean bool)
	{
		if(bool)
		{
			return 1.0f;
	...
	public float normalizeNumberOfPills(int numOfPills)
	{
		return ((numOfPills-0)/(this.numberOfTotalPillsInLevel-0))*(1-0)+0;
	}
	
	public float normalizeNumberOfPowerPills(int numOfPowerPills)
	{
		return ((numOfPowerPills-0)/(this.numberOfTotalPowerPillsInLevel-0))*(1-0)+0;
	}
	
	public float normalizeTotalGameTime(int time)
	{
		return ((time-0)/(Constants.MAX_TIME-0))*(1-0)+0;
	}
	
	public float normalizeCurrentLevelTime(int time)
	{
		return ((time-0)/(Constants.LEVEL_LIMIT-0))*(1-0)+0;
	}
	//Normalises the direction between 0 and 0.75.
	//This class is custom
	public float normalizeDirection(MOVE move){
		float result = 0;
	 ...
	 * @return
	 */
	public float normalizeCurrentScore(int score)
	{
